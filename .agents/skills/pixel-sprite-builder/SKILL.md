...
본 프로젝트는 외부 그래픽 툴이나 이미지 파일 의존 없이, 순수 Node.js 환경에서 HTML5 Canvas 2D API를 이용해 94종의 레트로 픽셀아트 PNG를 직접 굽는 자체 에셋 파이프라인([`generate_assets.js`](generate_assets.js))을 채택하고 있습니다.

---

## 1. 픽셀아트 규격 및 해상도 표준

| 용도 | 캔버스 해상도 | 권장 도트 픽셀 크기 | 파일 저장 위치 |
| :--- | :--- | :--- | :--- |
| **무기/패시브 아이콘** | 32 x 32 또는 48 x 48 | 1~2px 단위 도트 | `assets/sprites/icon_*.png` |
| **플레이어 캐릭터** | 32 x 32 | 1px 단위 도트 | `assets/sprites/player_*.png` |
| **일반 몬스터** | 32 x 32 | 1~2px 단위 도트 | `assets/sprites/enemy_*.png` |
| **보스 마물** | 64 x 64 | 2px 단위 도트 | `assets/sprites/boss_*.png` |
| **바닥 타일/장애물** | 64 x 64 | 2px 단위 도트 | `assets/sprites/tile_*.png` |
...
1. [`generate_assets.js`](generate_assets.js) 열기.
2. 새 도트 그리기 함수 작성:
   ```javascript
   function generateMyWeaponIcon() {
     const canvas = createCanvas(48, 48);
     const ctx = canvas.getContext('2d');
     ctx.imageSmoothingEnabled = false;

     // 도트 픽셀 채우기 (fillRect 활용)
     ctx.fillStyle = '#facc15';
     ctx.fillRect(20, 10, 8, 28);
     // ...
     saveCanvasToPng(canvas, 'assets/sprites/icon_myweapon.png');
   }
   ```
3. `main()` 실행 함수에 등록 후 터미널에서 빌드 실행:
   ```bash
